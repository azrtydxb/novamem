  ...
  const {
    ...
    queryFn: async () => {
      ...
      return isAdmin
        ...
        : ({
            kind: "user",
            data: r.body as UserMetricsSnapshot,
          } as AnySnapshot);
    },
  ...
  useEffect(() => {
    ...
    setHistory((cur) => {
      ...
      const point: HistoryPoint = {
        ...
        warmHits: prev
          ? snap.data.counters.hits_warm_total - prev.hits_warm_total
          : 0,
        coldHits: prev
          ? snap.data.counters.hits_cold_total - prev.hits_cold_total
          : 0,
        graphHits: prev
          ? snap.data.counters.hits_graph_total - prev.hits_graph_total
          : 0,
        byToken,
      };
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          {isAdmin && adminGauges ? (
            <StatCard
              ...
              label="Last decay"
              value={
                adminGauges.last_decay_run_iso
                  ? fmtRelative(adminGauges.last_decay_run_iso)
                  : "never"
              }
              sublabel={
                isAdminSnapshot(snap)
                  ? `${fmtNumber(
                      snap.data.counters.decay_runs_total
                    )} total runs`
                  : ""
              }
            />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Queries and remembers per second, last{" "}
              {HISTORY_POINTS * (POLL_MS / 1000)}s.
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                How much each tier contributed to fused results.
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {isAdmin
                  ? "Live counts sampled at request time."
                  : "Counts for you only."}
              </CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-2 gap-3">
              ...
              {adminGauges ? (
                <StatCard
                  ...
                  value={fmtNumber(adminGauges.orphans_pending)}
                  icon={Trash2}
                />
              ) : null}
            </CardContent>
        ...
        {isAdminSnapshot(snap) ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Memory transitions and cleanup events (cross-user).
              </CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-2 lg:grid-cols-5 gap-3">
              <StatCard
                label="Remembers"
                value={fmtNumber(snap.data.counters.remembers_total)}
                icon={Zap}
              />
              <StatCard
                label="Forgets"
                value={fmtNumber(snap.data.counters.forgets_total)}
                icon={Trash2}
              />
              <StatCard
                label="Promotions"
                value={fmtNumber(snap.data.counters.promotions_total)}
                icon={ArrowUpToLine}
                tone="graph"
              />
              <StatCard
                label="Demotions"
                value={fmtNumber(snap.data.counters.demotions_total)}
                icon={ArrowDownToLine}
                tone="warm"
              />
              <StatCard
                label="Orphans reaped"
                value={fmtNumber(snap.data.counters.orphans_reaped_total)}
                icon={Trash2}
              />
            </CardContent>
          </Card>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Queries and remembers per minute, last 24 hours. Persisted across
          reboots.
          {isFetching ? " · refreshing…" : ""}{" "}
          {points.length > 0
            ? `Total: ${fmtNumber(totalQueries)} queries / ${fmtNumber(
                totalRemembers
              )} remembers.`
