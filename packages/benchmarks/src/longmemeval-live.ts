...
export function normalizeLongMemEvalCutoffs(
  cutoffs: number[],
  maxNovaMemK = 200
): number[] {
  ...
  if (tooLarge.length > 0) {
    throw new Error(
      `Cannot report ${tooLarge
        .map((k) => `top_${k}`)
        .join(
          ", "
        )}: NovaMem /v1/search currently enforces k <= ${maxNovaMemK}. Change the API limit or use a separate retrieval path first.`
    );
  }
  return unique;
}
...
export function chunkLongMemEvalSession(
  input: LongMemEvalChunkInput,
  turnsPerChunk = 2
  ...
  for (let i = 0; i < turns.length; i += turnsPerChunk) {
    ...
    chunks.push({
      content: `[LongMemEval question=${input.questionId} session=${
        input.sessionId
      } chunk=${chunkIndex}]\n${formatTurns(
        input.sessionId,
        input.date,
...
function formatMemories(
  ...
  for (const [index, result] of memories.entries()) {
    ...
    if (memory.length > maxCharsEach)
      memory = `${memory.slice(0, maxCharsEach)}…`;
...
export function buildLongMemEvalAnswerPrompt(opts: {
  ...
  const memories = formatMemories(
    opts.memories,
    opts.maxCharsEach ?? 8_000,
    opts.maxTotalChars ?? 180_000
  );
...
export function buildLongMemEvalJudgePrompt(opts: {
  question: string;
  answer: string;
  hypothesis: string;
}): string {
...
export function parseJudgeJson(text: string): {
  ...
  if (candidate) {
    try {
      ...
      const scoreNum = Number(
        parsed.score ?? (judgmentText === "PASS" ? 1 : 0)
      );
      const score: 0 | 1 = judgmentText === "PASS" || scoreNum >= 0.5 ? 1 : 0;
      return {
        judgment: score === 1 ? "PASS" : "FAIL",
        score,
  ...
  const score: 0 | 1 =
    upper.includes("PASS") && !upper.includes("FAIL") ? 1 : 0;
  return {
    judgment: score === 1 ? "PASS" : "FAIL",
    score,
