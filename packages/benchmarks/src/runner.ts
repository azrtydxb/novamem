  ...
  return fixture.memories
    ...
    .map((m): RetrievedMemory => {
      ...
      const exactExpected =
        ...
        m.text.toLowerCase().includes(query.expectedAnswer.toLowerCase())
          ? 100
          : 0;
      return {
        id: m.id,
        text: m.text,
        score: exactExpected + overlap / Math.max(q.size, 1),
        answer:
          query.expectedAnswer &&
          m.text.toLowerCase().includes(query.expectedAnswer.toLowerCase())
            ? query.expectedAnswer
  ...
  const answer = {
    exactMatch:
      answeredCount === 0
        ? 0
        : answerable.reduce(
            (sum, c) => sum + exactMatch(c.answer, c.expectedAnswer),
            0
          ) / answeredCount,
    tokenF1:
      answeredCount === 0
        ? 0
        : answerable.reduce(
            (sum, c) => sum + tokenF1(c.answer, c.expectedAnswer),
            0
          ) / answeredCount,
    answeredCount,
  };
    ...
    forbiddenHitRateAtK[k] =
      cases.length === 0
        ...
        : cases.reduce((sum, c) => {
            ...
            return (
              sum +
              (c.retrievedIds.slice(0, k).some((id) => forbidden.has(id))
                ? 1
                : 0)
            );
          }, 0) / cases.length;
