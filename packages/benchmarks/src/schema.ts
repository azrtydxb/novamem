...
export const benchmarkKindSchema = z.enum([
  "longmemeval",
  "locomo",
  "beir",
  "rag",
...
export const benchmarkMemorySchema = z
  .object({
    id: z.string().min(1),
    text: z.string().min(1),
    ...
    for (const query of fixture.queries) {
      for (const id of query.relevantMemoryIds) {
        if (!ids.has(id))
          ctx.addIssue({
            code: "custom",
            path: ["queries", query.queryId, "relevantMemoryIds"],
            message: `unknown relevant memory id: ${id}`,
          });
      }
      for (const id of query.forbiddenMemoryIds ?? []) {
        if (!ids.has(id))
          ctx.addIssue({
            code: "custom",
            path: ["queries", query.queryId, "forbiddenMemoryIds"],
            message: `unknown forbidden memory id: ${id}`,
          });
      }
    }
  });
