...
async function request(baseUrl: string, token: string, method: string, path: string, body?: unknown): Promise<any> {
...
function extractSearchResults(payload: any): Array<{ id: string; content?: string; score?: number }> {
  const rows = Array.isArray(payload?.results) ? payload.results : Array.isArray(payload) ? payload : [];
  return rows.map((r: any) => ({ id: String(r.id), content: r.content ?? r.text, score: typeof r.score === "number" ? r.score : undefined }));
}

async function createBenchmarkProject(fixture: BenchmarkFixture, opts: LiveOptions): Promise<{ project: string; created: boolean }> {
  if (opts.createProject) {
    const created = await request(opts.baseUrl, opts.token, "POST", "/v1/me/projects", { name: opts.project ?? `bench-${fixture.name}-${Date.now()}` });
    return { project: String(created.id), created: true };
  }
  if (!opts.project) throw new Error("live benchmark requires --project or --create-project");
  return { project: opts.project, created: false };
}

async function cleanupSeededMemories(opts: LiveOptions & { project: string }, ids: string[]): Promise<string[]> {
  const failures: string[] = [];
  for (const id of ids) {
    try {
      await request(opts.baseUrl, opts.token, "POST", "/v1/forget", { id, project: opts.project });
    } catch (err) {
...
export async function seedFixtureIntoNovaMem(fixture: BenchmarkFixture, opts: LiveOptions & { project: string }): Promise<{ project: string; idMap: Map<string, string> }> {
  ...
  for (const memory of [...fixture.memories].sort((a, b) => {
    ...
    const stored = await request(opts.baseUrl, opts.token, "POST", "/v1/capture", {
      content: `[bench:${memory.id}] ${memory.text}`,
      namespace: opts.namespace,
      project: opts.project,
      force: true,
      metadata: { benchmarkFixture: fixture.name, benchmarkMemoryId: memory.id, ...(memory.metadata ?? {}) },
...
export function makeNovaMemRetriever(opts: LiveOptions & { project: string; idMap: Map<string, string> }): Retriever {
  const reverse = new Map([...opts.idMap.entries()].map(([fixtureId, storedId]) => [storedId, fixtureId]));
  return async (query: BenchmarkQuery, _fixture, { k }): Promise<RetrievedMemory[]> => {
    const payload = await request(opts.baseUrl, opts.token, "POST", "/v1/search", {
      query: query.text,
      k,
      namespace: opts.namespace,
      project: opts.project,
    ...
    return extractSearchResults(payload).map((r) => {
      const fixtureId = reverse.get(r.id) ?? /\[bench:([^\]]+)\]/.exec(r.content ?? "")?.[1] ?? r.id;
      return { id: fixtureId, text: r.content, score: r.score, answer: query.expectedAnswer && (r.content ?? "").toLowerCase().includes(query.expectedAnswer.toLowerCase()) ? query.expectedAnswer : undefined };
    });
  };
}
  ...
  try {
    ...
    if (opts.cleanup) {
      if (projectState.created) {
        try {
          await request(opts.baseUrl, opts.token, "DELETE", `/v1/me/projects/${encodeURIComponent(projectState.project)}`);
        } catch (err) {
          cleanupFailures.push(`project ${projectState.project}: ${err instanceof Error ? err.message : String(err)}`);
        }
      } else {
        cleanupFailures.push(...await cleanupSeededMemories(liveOpts, [...idMap.values()]));
      }
      if (cleanupFailures.length > 0) {
        throw new Error(`live benchmark cleanup failed: ${cleanupFailures.join("; ")}`);
      }
    }
  }
}
