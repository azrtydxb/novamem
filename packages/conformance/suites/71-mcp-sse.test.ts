...
describe("MCP legacy SSE transport", () => {
  it("handshake advertises the same tool surface; a tool call round-trips", async () => {
    ...
    try {
      ...
      let buf = "";
      const readUntil = async (pred: (buf: string) => boolean): Promise<void> => {
        const deadline = Date.now() + 20_000;
        while (!pred(buf)) {
          if (Date.now() > deadline) throw new Error(`SSE frame timeout; buffer: ${buf.slice(-400)}`);
      ...
      const post = async (body: unknown): Promise<void> => {
        const r = await fetch(`${env.url}/mcp/messages?sessionId=${sessionId}`, {
          method: "POST",
          headers: {
      ...
      const messageFor = (id: number): { result?: unknown; error?: unknown } | undefined => {
        const frames = buf.split("\n\n");
        for (const f of frames) {
          const data = f.split("\n").filter((l) => l.startsWith("data: ")).map((l) => l.slice(6)).join("");
          if (!data) continue;
          try {
            const msg = JSON.parse(data) as { id?: number; result?: unknown; error?: unknown };
      ...
      const resultFor = (id: number): unknown => {
        ...
        if (msg?.error) throw new Error(`JSON-RPC error for id ${id}: ${JSON.stringify(msg.error)}`);
        return msg?.result;
      };

      await post({
        jsonrpc: "2.0", id: 1, method: "initialize",
        params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "conf-sse", version: "0" } },
      });
      ...
      if (existsSync(SNAPSHOT)) {
        const snapshot = JSON.parse(readFileSync(SNAPSHOT, "utf8")) as { names: string[] };
      ...
      await post({ jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "memory_recent", arguments: { k: 1, namespace: "default" } } });
