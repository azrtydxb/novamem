  ...
  for (const id of createdUserIds) {
    try {
      ...
      if (r.status !== 200 && r.status !== 404) {
        console.warn(`cleanup: delete admin-provisioned user ${id} → ${r.status}`, r.body);
      }
    } catch (e) {
...
describe.skipIf(env.authMode !== "user" || !hasAdminIdentity)("admin plane (user mode)", () => {
  it("unauthenticated is 401 across every requireAdmin-gated /v1/admin/* route", async () => {
    ...
    const auditLog = await api<z.infer<typeof ErrorBody>>("/v1/admin/audit-log", noAuth);
    expect(auditLog.status).toBe(401);
    expect(auditLog.body.error).toBe("unauthorized");

    const healthDeep = await api<z.infer<typeof ErrorBody>>("/v1/admin/health/deep", noAuth);
    expect(healthDeep.status).toBe(401);
    expect(healthDeep.body.error).toBe("unauthorized");

    const revoke = await api<z.infer<typeof ErrorBody>>("/v1/admin/tokens/revoke", {
      ...noAuth,
      method: "POST",
      body: { token: "nm_bogus" },
    });
    ...
    const createUser = await api<z.infer<typeof ErrorBody>>("/v1/admin/users", {
      ...noAuth,
    ...
    const deleteUser = await api<z.infer<typeof ErrorBody>>("/v1/admin/users/bogus-id", {
      ...noAuth,
      method: "DELETE",
    });
    ...
    const putQuota = await api<z.infer<typeof ErrorBody>>("/v1/admin/users/bogus-id/quota", {
      ...noAuth,
      method: "PUT",
      body: {},
    });
  ...
  it("GET /v1/admin/users (admin cookie): bootstrap admin appears with an admin role", async () => {
    const r = await adminCookieApi<z.infer<typeof AdminUsersResponse>>("/v1/admin/users");
  ...
  it("full lifecycle: provision throwaway non-admin user → prove non-admin denial → quota → audit-log → tokens/revoke → delete", async () => {
    ...
    const create = await adminCookieApi<z.infer<typeof AdminCreateUserResponse>>(
      "/v1/admin/users",
      {
        body: {
          email,
    ...
    const deniedUsers = await api<z.infer<typeof ErrorBody>>("/v1/admin/users", { token: nonAdminToken });
    expect(deniedUsers.status).toBe(403);
    ErrorBody.parse(deniedUsers.body);
    expect(deniedUsers.body.error).toBe("admin only");

    const deniedAudit = await api<z.infer<typeof ErrorBody>>("/v1/admin/audit-log", {
      token: nonAdminToken,
    });
    expect(deniedAudit.status).toBe(403);
    expect(deniedAudit.body.error).toBe("admin only");

    const deniedRevoke = await api<z.infer<typeof ErrorBody>>("/v1/admin/tokens/revoke", {
      method: "POST",
      token: nonAdminToken,
      body: { token: "nm_bogus" },
    });
    ...
    const deniedCreate = await api<z.infer<typeof ErrorBody>>("/v1/admin/users", {
      token: nonAdminToken,
      body: { email: "should-not-be-created@example.com", password: "irrelevant" },
    });
    expect(deniedCreate.status).toBe(403);
    expect(deniedCreate.body.error).toBe("admin only");

    const deniedDelete = await api<z.infer<typeof ErrorBody>>(`/v1/admin/users/${userId}`, {
      method: "DELETE",
      token: nonAdminToken,
    });
    ...
    const deniedQuota = await api<z.infer<typeof ErrorBody>>(`/v1/admin/users/${userId}/quota`, {
      method: "PUT",
      token: nonAdminToken,
      body: {},
    });
    ...
    const deniedHealthDeep = await api<z.infer<typeof ErrorBody>>("/v1/admin/health/deep", {
      token: nonAdminToken,
    });
    ...
    expect(setQuota.body.quota).toEqual({ maxEntries: 50, writesPerMinute: 5 });

    // ── 4. GET /v1/admin/health/deep (admin cookie) ───────────────────
    const health = await adminCookieApi<z.infer<typeof AdminHealthDeepResponse>>(
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      const audit = await adminCookieApi<z.infer<typeof AdminAuditLogResponse>>(
    ...
    expect(
      auditEntries.some((e) => e.action === "admin.user.create" && e.target === userId),
    ).toBe(true);
    expect(
      auditEntries.some((e) => e.action === "admin.user.quota" && e.target === userId),
    ).toBe(true);
    ...
    const mint = await adminCookieApi<z.infer<typeof MintTokenResponse>>("/v1/me/tokens", {
    ...
    const revokeAgain = await adminCookieApi<z.infer<typeof AdminRevokeResponse>>(
      "/v1/admin/tokens/revoke",
      {
        method: "POST",
        body: { token: plaintext },
    ...
    const revokeGarbage = await adminCookieApi<z.infer<typeof AdminRevokeResponse>>(
      "/v1/admin/tokens/revoke",
      {
        method: "POST",
    ...
    const dryRun = await adminCookieApi<{ dryRun: boolean; wouldDelete: { userId: string } }>(
      `/v1/admin/users/${userId}?dryRun=true`,
      { method: "DELETE" },
    );
    ...
    const deleteAgain = await adminCookieApi<z.infer<typeof ErrorBody>>(`/v1/admin/users/${userId}`, {
      method: "DELETE",
    });
    ...
    const bootstrap = await adminCookieApi<z.infer<typeof AdminUsersResponse>>(
      "/v1/admin/users",
    );
    AdminUsersResponse.parse(bootstrap.body);
    const adminId = bootstrap.body.users.find((u) => u.email === env.adminEmail)!.id;
    const selfDelete = await adminCookieApi<z.infer<typeof ErrorBody>>(`/v1/admin/users/${adminId}`, {
      method: "DELETE",
    });
