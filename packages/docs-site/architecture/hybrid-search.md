...
That fallback matters because of how this tier is actually driven: `memory_context` passes the **entire user message** as the query. Under pure AND semantics a stored fact like _"NovaMem runs on port 7778"_ could never match _"what port does the novamem deployment run on in production"_ — every one of those lexemes had to appear in one row. The tier returned nothing on the primary grounding path, and hybrid search silently degraded to vector-only.

## Score scales

The two signals are _not_ the same kind of number, so they are not treated the same way:
...
Candidates are over-fetched (3× `k`) _before_ superseded and sensitivity-hidden entries are filtered out, so `k` is `k` visible results. Filtering after the cut used to shrink a top-10 containing four superseded rows down to six.
