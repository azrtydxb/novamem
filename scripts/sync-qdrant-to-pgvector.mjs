...
const pool = new pg.Pool({ connectionString: process.env.NOVAMEM_WARM_URL, max: 4 });
...
const cols = (await (await fetch(`${QD}/collections`)).json()).result.collections.map((c) => c.name);
let copied = 0;
let orphans = 0;
for (const col of cols) {
  let offset = null;
  do {
    const body = { limit: 512, with_payload: true, with_vector: true, ...(offset ? { offset } : {}) };
    const r = await (await fetch(`${QD}/collections/${encodeURIComponent(col)}/points/scroll`, {
    ...
    for (const p of pts) {
      ...
      const scope = m.project_id === null ? `u:${m.user_id}` : `p:${m.project_id}`;
      values.push(`($${++n}, $${++n}, $${++n}, $${++n}, $${++n}, $${++n}::vector, $${++n})`);
      params.push(eid, m.user_id, m.project_id, m.namespace, scope,
        `[${p.vector.join(",")}]`, JSON.stringify(p.payload ?? {}));
      copied++;
    }
    if (values.length) {
      await pool.query(
