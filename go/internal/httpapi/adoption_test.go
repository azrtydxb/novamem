...
func TestAdoptionReportShape(t *testing.T) {
	...
	mcp := report["mcp"].(map[string]any)
	if mcp["toolCount"] != 21 {
		t.Fatalf("toolCount %v, want 21 (14 memory_* + 7 project_*)", mcp["toolCount"])
	}
	if report["requestedClient"] != "generic" {
		t.Fatalf("requestedClient %v", report["requestedClient"])
	}
	diags := report["diagnostics"].([]map[string]any)
	if len(diags) != 3 || diags[0]["status"] != "unknown" || diags[0]["ok"] != false {
	...
	diags = report["diagnostics"].([]map[string]any)
	if diags[0]["status"] != "ok" || diags[0]["ok"] != true || diags[0]["action"] != "none" {
