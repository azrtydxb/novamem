...
// cap + idle reaping, and the Streamable HTTP transport (dual-era; the
// deprecated HTTP+SSE pair was removed in ADR 0007). The official go-sdk
// was evaluated and skipped: its
...
// Generated by cmd/gen-contract from api/openapi.yaml — the tool
// surface is the `x-mcp-tool` extension on the operation each tool
// backs, so a tool cannot exist without a route to serve it. Originally
// transcribed from (packages/server/src/mcp-tools.ts TOOL_DEFINITIONS) via:
//
