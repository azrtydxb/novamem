...
func run() error {
	...
	// warm pool exactly as main.ts passes it to WarmStore. config.Load
	// already bounds it to [1, MaxInt32]; re-checking here keeps the
	// narrowing conversion provably safe at its own call site rather
	// than relying on a caller two packages away.
	if cfg.PgPoolMax < 1 || cfg.PgPoolMax > math.MaxInt32 {
		return fmt.Errorf("NOVAMEM_PG_POOL_MAX out of range: %d", cfg.PgPoolMax)
	}
